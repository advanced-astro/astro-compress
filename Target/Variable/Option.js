var n = (await import("../Function/Merge.js")).default(
	(await import("@playform/pipe/Target/Variable/Option.js")).default,
	{
		...
		Action: {
			Failed: async ({ Input: t }) =>
				`${r("Error:")} Cannot compress file ${e(await o(t))}${r((await import("path")).parse(t).base)}`,
			Passed: async ({ Before: t, Buffer: a }) =>
				t > s.byteLength(a.toString()),
			Accomplished: async ({ Input: t, Before: a, After: f }) => {
				const i = a - f;
				return `${e(`(-${(await import("@playform/pipe/Target/Function/Bytes.js")).default(i)})`)}	${(await import("kleur/colors")).green(`${((i / a) * 100).toFixed(2)}%`)} reduction in ${e(await o(t))}${(await import("kleur/colors")).cyan((await import("path")).parse(t).base)}`;
			},
			Changed: async (t) =>
...
const { gray: e, red: r } = await import("kleur/colors"),
